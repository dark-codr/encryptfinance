...
class UserUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["profileform"] = self.second_form_class(self.request.GET, instance=self.request.user)
        return context

    def form_valid(self, form):
        object = self.get_object()
        form = self.form_class(self.request.POST, instance=self.request.user)
        profileform = self.second_form_class(self.request.POST, instance=self.request.user.userprofile)
